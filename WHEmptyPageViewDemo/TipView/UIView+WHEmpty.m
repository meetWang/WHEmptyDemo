...
- (void)showNetErrorPageViewWithFrame:(CGRect)rect{
    ...
    if (!self.netErrorPageView) {
        self.netErrorPageView = [[CDMNetErrorPageView alloc]initWithFrame:rect];
        if (self.reloadAction) {
            self.netErrorPageView.didClickReloadBlock = self.reloadAction;
        }
    }
...
- (void)showBlankPageViewWithFrame:(CGRect)rect{
    ...
    if (!self.blankPageView) {
        self.blankPageView = [[CDMBlankPageView alloc]initWithFrame:rect];
    }
    [self addSubview:self.blankPageView];
    [self bringSubviewToFront:self.blankPageView];
}
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        UIImageView* errorImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"nonet"]];
...
- (instancetype)initWithFrame:(CGRect)frame{
    self = [super initWithFrame:frame];
    if (self) {
        UIImageView* nodataImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"nodata"]];
